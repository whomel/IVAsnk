#Run10
#snakemake --snakefile who_flu.py -j 48 --config ifq=/Data-RAID5/iseq/raw/illumina/iSeq10/ out=output/iSeq10analysisIndel/
...
#Pipeline designed to obtain consensus secuencies from raw fastq data using denovo assemblies.
...
#Prepare final output separated by gene
...
#Denovo assembly using IVA paired.
rule iva:
    input:
        R1out=workspace+"qualtrim/{sample}.R1.paired.fastq",
        R2out=workspace+"qualtrim/{sample}.R2.paired.fastq"
...
#Final consensus without discarding indels from bam, but filtering the vcf using `tools/filterVCF.py`
rule FINALconsensus:
    input:
        bamNoInd=workspace+'alignments/post/{sample}.noInd.bam',
        bam=workspace+'alignments/post/{sample}.bam',
    ...
    output:
        ...
        vcfilt=workspace+'alignments/post/{sample}.ind.filt.vcf',
