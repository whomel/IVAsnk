...
for filename in glob.glob(sys.argv[1]+"/*.fa"):
    with open (filename,'r') as fi:
        ...
        if fluType=="B":
            ...
            output, error = process.communicate()
	    #Rename "_HA" to ".4"
            for index,record in enumerate(SeqIO.parse(sys.argv[1] +"/rename/type/FLUB/"+bname, 'fasta')):
                res+=">"+record.id.split("_")[0]+"."+segN[record.id.split("_")[1]]+"\n"+record.seq+"\n"
            with open (sys.argv[1] +"/rename/type/FLUB/"+bname,"w") as fo:
                fo.write(str(res))
        elif fluType=="A":
            bname = os.path.basename(filename)
            #Create folder
            ...
            output, error = process.communicate()
	    #Rename "_HA" to ".4"
            for index,record in enumerate(SeqIO.parse(sys.argv[1] +"/rename/type/FLUA/"+subtypeHA+subtypeNA+"/"+bname, 'fasta')):
                res+=">"+record.id.split("_")[0]+"."+segN[record.id.split("_")[1]]+"\n"+record.seq+"\n"
            with open (sys.argv[1] +"/rename/type/FLUA/"+subtypeHA+subtypeNA+"/"+bname,"w") as fo:
                fo.write(str(res))
